...
export const register = async (req, res) => {
  ...
  if (!email || !password || !nombre_completo) {
    return res
      .status(400)
      .json({ error: "Faltan campos (email, password, nombre_completo)." });
  }
  ...
  const roleToAssign =
    rol && ["ADMIN", "OPERADOR", "VISUALIZADOR"].includes(rol)
      ? rol
      : "VISUALIZADOR";
  ...
  try {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: process.env.VERCEL_URL || "http://localhost:5173",
      },
    });
    ...
    res
      ...
      .json({
        message:
          "Registro en proceso. Verifica tu correo si así está configurado.",
      });
  } catch (error) {
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (userData && !userData.activo) {
      return res
        .status(403)
        .json({ error: "Usuario inactivo. Contacta administración." });
    }
